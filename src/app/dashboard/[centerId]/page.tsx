...
export default async function CenterDashboardPage({
  params,
}: CenterDashboardPageProps) {
  const { centerId } = await params;
  return (
    <div className="space-y-8">
      {/* Breadcrumb */}
      <CenterBreadcrumb centerId={centerId} />

      {/* Page header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900">센터 대시보드</h1>
        <p className="text-gray-600 mt-2">센터 운영 현황을 한눈에 확인하세요</p>
      </div>

      {/* Stats overview */}
      <Suspense fallback={<StatsLoading />}>
        <DashboardOverview centerId={centerId} />
      </Suspense>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Upcoming classes */}
        <Suspense fallback={<ActivityLoading />}>
          <UpcomingClasses centerId={centerId} />
        </Suspense>
      </div>
    </div>
  );
